...
class ToCompare extends Model
{
     ...
     *
     */
    public function compareFiles($aa_id,$q_id) {
        $file1ReturnArray = Array();
        $file2ReturnArray = Array();

        $realAnswer = Question::find($q_id);
        $s_ans = AssessmentAnswer::find($aa_id);
//        dd($realAnswer->answer);

        $mainFileArray = explode("\n",$s_ans->s_answer);
        $duplicateFileArray = explode("\n",$realAnswer->answer);
    ...
    public function createIFrames($file1Contents, $file2Contents) {
        ...
        $html1Code = "<html> <body bgcolor='#ffffff'>";
        ...
        $html2Code = "<html> <body bgcolor='#ffffff'>";
